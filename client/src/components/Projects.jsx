...
const CATEGORY_COLOR = {
  fullstack: 'from-accent/25 to-blue-500/10 border-accent/25',
  ml: 'from-cyan-accent/25 to-teal-500/10 border-cyan-accent/25',
};
const ACCENT_COLOR = { fullstack: 'text-accent-light', ml: 'text-cyan-accent' };
const ICON_BG = {
  fullstack: 'bg-accent/10 group-hover:bg-accent/20',
  ml: 'bg-cyan-accent/10 group-hover:bg-cyan-accent/20',
};
const BORDER_GLOW = {
  fullstack: 'hover:shadow-[0_0_30px_rgba(124,90,240,0.15)]',
  ml: 'hover:shadow-[0_0_30px_rgba(34,211,238,0.15)]',
};
const STATUS_COLOR = {
  Completed: 'bg-green-500/15 text-green-400 border-green-500/20',
  'In Progress': 'bg-yellow-500/15 text-yellow-400 border-yellow-500/20',
  'Open Source': 'bg-blue-500/15 text-blue-400 border-blue-500/20',
};
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg font-mono text-xs transition-all duration-300 ${
                activeTab === tab.key
                  ...
                  : 'glass text-text-muted hover:text-text-secondary hover:border-border-light'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="flex flex-col gap-8">
          <AnimatePresence mode="wait">
            {filtered.map((project, i) => {
              ...
              const accentColor =
                ACCENT_COLOR[project.category] || 'text-accent-light';
              const iconBg = ICON_BG[project.category] || 'bg-accent/10';
              const borderGlow =
                BORDER_GLOW[project.category] || '';
              const statusStyle =
                STATUS_COLOR[project.status] || STATUS_COLOR['Completed'];
              ...
              return (
                <motion.article
                  ...
                  className={`glass rounded-2xl p-8 md:p-10 hover:border-white/10 transition-all duration-500 group relative overflow-hidden ${borderGlow}`}
                >
                      ...
                      <div className="flex items-start justify-between gap-4 mb-1">
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <span
                              className={`font-mono text-xs ${accentColor} tracking-widest`}
                            >
                            ...
                            {project.status && (
                              <span
                                className={`font-mono text-[10px] px-2 py-0.5 rounded-full border ${statusStyle}`}
                              >
                                {project.status}
                              </span>
                            )}
